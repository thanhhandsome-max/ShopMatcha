...
export async function registerCustomer(data: RegisterRequest): Promise<AuthResponse> {
  try {
    ...
    const tokenPayload = {
      ...
      MaVaiTro: result.account.MaVaiTro || 'VT03',
      role: result.account.vaitro?.TenVaiTro || 'Khách hàng'
    };
...
export async function loginUser(data: LoginRequest): Promise<AuthResponse> {
  try {
    ...
    // Verify password
    const passwordValid = await verifyPassword(data.password, account.MatKhau);
    if (!passwordValid) {
    ...
    if (account.khachhang.length > 0) {
      customerInfo = account.khachhang[0];
    }
    ...
    const tokenPayload = {
      ...
      MaVaiTro: account.MaVaiTro || 'VT03',
      role: account.vaitro?.TenVaiTro || 'Khách hàng'
    };
